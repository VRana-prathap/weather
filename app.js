const BACKEND_URL = 'https://weather-d1sm.onrender.com';
...
async function fetchWeatherData(city) {
  try {
    ...
    renderCurrentWeather(data.current, data.location);
    renderForecast(data.forecast, data.current.timezone);
  } catch (err) {
    console.error("Fetch failed:", err);
    alert(err.message || "Weather data unavailable. Try again.");
  }
}
...
function renderCurrentWeather(data, location) {
  ...
  document.getElementById('locationHeader').innerHTML = `${location.name} ${getFlagEmoji(country)} &nbsp; • &nbsp; ${country}`;
